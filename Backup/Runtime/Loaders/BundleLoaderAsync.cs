...
namespace vFrame.Bundler.Loaders
{
    public class BundleLoaderAsync : BundleLoaderBase, IAsync, IAsyncProcessor
    {
        ...
        public IEnumerator OnAsyncProcess() {
            ...
            UnityEngine.Logger.LogInfo("Bundle load async start: {0}", _path);
            ...
            UnityEngine.Logger.LogInfo("Bundle load request finished: {0}", _path);

            LoadAndCache(_bundleLoadRequest);

            UnityEngine.Logger.LogInfo("AssetBundle asynchronously loading finished, path: {0}", _path);
        ...
        public void ForceLoadComplete() {
            ...
            UnityEngine.Logger.LogInfo("Force load asset bundle: {0}", _path);

            if (null == _bundleLoadRequest) {
                UnityEngine.Logger.LogInfo("Force load asset bundle, but load request not exist, create it: {0}", _path);
                _bundleLoadRequest = CreateBundleLoadRequest();
            }
            LoadAndCache(_bundleLoadRequest);
        }
        ...
        protected override void OnLoadProcess() {
            UnityEngine.Logger.LogInfo("Start asynchronously loading process: {0}", _path);
        ...
        private AssetBundleCreateRequest CreateBundleLoadRequest() {
            UnityEngine.Logger.LogInfo("Bundle load request does not exist, create it from file: {0}", _path);
