    ...
    public class BundlerMessenger : MonoBehaviour
    {
        ...
        private void Awake() {
            ...
            if (!string.IsNullOrEmpty(_instanceId)) {
                if (!RecoverRefs()) {
                    UnityEngine.Logger.LogError("BundlerMessenger:Awake - Recover refs failed, see console output!");
                }
            }
            _instanceId = GetInstanceID().ToString();

            UnityEngine.Logger.LogVerbose("BundlerMessenger::Awake: {0}", this);
        ...
        private bool RecoverRefs() {
            UnityEngine.Logger.LogVerbose("BundlerMessenger::RecoverRefs: {0}", this);

            var clonedFromMessenger = FindMessengerInstance(_instanceId);
            if (!clonedFromMessenger) {
                UnityEngine.Logger.LogError("BundlerMessenger::RecoverRefs: Messenger not found, instance id: {0}", _instanceId);
                return false;
            }
            ...
            if (null != clonedFromMessenger._assets) {
                foreach (var asset in clonedFromMessenger._assets) {
                    UnityEngine.Logger.LogVerbose("BundlerMessenger::RecoverRefs: {0}, loader: {1}", this, asset.Loader);
                    asset.Retain();
                    UnityEngine.Logger.LogVerbose("BundlerMessenger::RecoverRefs: {0}, loader: {1}, finished!", this, asset.Loader);
                    _assets.Add(asset);
                }
            }
            ...
            if (null != clonedFromMessenger._typedAssets) {
                foreach (var kv in clonedFromMessenger._typedAssets) {
                    UnityEngine.Logger.LogVerbose("BundlerMessenger::RecoverRefs: {0}, loader: {1}", this, kv.Value.Loader);
                    kv.Value.Retain();
                    UnityEngine.Logger.LogVerbose("BundlerMessenger::RecoverRef: {0}, loader: {1}, finished", this, kv.Value.Loader);
                    _typedAssets.Add(kv.Key, kv.Value);
                }
            }

            UnityEngine.Logger.LogVerbose("BundlerMessenger::RecoverRef: {0}, finished!", this);
        ...
        private void OnDestroy() {
            UnityEngine.Logger.LogVerbose("BundlerMessenger::OnDestroy: {0}", this);
            ReleaseRef();
        }
        ...
        public void ReleaseRef() {
            if (null != _assets) {
                foreach (var asset in _assets) {
                    UnityEngine.Logger.LogVerbose("BundlerMessenger::ReleaseRef: {0}, loader: {1}", this, asset.Loader);
                    asset.Release();
                    UnityEngine.Logger.LogVerbose("BundlerMessenger::ReleaseRef: {0}, loader: {1}, finished!", this, asset.Loader);
                }
            ...
            if (null != _typedAssets) {
                foreach (var kv in _typedAssets) {
                    UnityEngine.Logger.LogVerbose("BundlerMessenger::ReleaseRef: {0}, loader: {1}", this, kv.Value.Loader);
                    kv.Value.Release();
                    UnityEngine.Logger.LogVerbose("BundlerMessenger::ReleaseRef: {0}, loader: {1}, finished!", this, kv.Value.Loader);
                }
        ...
        public void RetainRef(AssetBase asset) {
            ...
            UnityEngine.Logger.LogVerbose("BundlerMessenger::RetainRef: {0}, loader: {1}", this, asset.Loader);

            asset.Retain();

            UnityEngine.Logger.LogVerbose("BundlerMessenger::RetainRef: {0}, loader: {1}, finished!", this, asset.Loader);
        ...
        public void RetainRef<TSetter>(AssetBase asset) {
            ...
            if (_typedAssets.ContainsKey(type)) {
                UnityEngine.Logger.LogVerbose("BundlerMessenger::RetainRef<T>: {0}, Setter exist: {1}, release previous asset: {2}",
                    this, type.FullName, _typedAssets[type].AssetPath);

                _typedAssets[type].Release();

                UnityEngine.Logger.LogVerbose("BundlerMessenger::RetainRef<T>: {0}, Setter exist: {1}, release previous asset: {2}, finished!",
                    this, type.FullName, _typedAssets[type].AssetPath);
            }

            _typedAssets[type] = asset;

            UnityEngine.Logger.LogVerbose("BundlerMessenger::RetainRef<T>: {0}, loader: {1}", this, asset.Loader);

            asset.Retain();

            UnityEngine.Logger.LogVerbose("BundlerMessenger::RetainRef<T>: {0}, loader: {1}, finished!", this, asset.Loader);
